...
XtslaTrain_HL = tslaX[trainIndexHL_Tsla,] 
XtslaTest_HL  = tslaX[-trainIndexHL_Tsla,]

XtslaTrain_SR = tslaX[trainIndexSR_Tsla,] 
XtslaTest_SR  = tslaX[-trainIndexSR_Tsla,]
...
YgoogTrain_HL      = tslaY$diffHighLowTomorrow[trainIndexHL_Goog]
YgoogTrain_SR      = tslaY$stockReturnTomorrow[trainIndexSR_Goog]
YgoogTrain_SRDummy = tslaY$stockReturnTomorrowDummy[trainIndexSR_Goog]

YgoogTest_HL       = tslaY$diffHighLowTomorrow[-trainIndexHL_Goog]
YgoogTest_SR       = tslaY$stockReturnTomorrow[-trainIndexSR_Goog]
YgoogTest_SRDummy  = tslaY$stockReturnTomorrowDummy[-trainIndexSR_Goog]


XgoogTrain_HL = tslaX[trainIndexHL_Goog,] 
XgoogTest_HL  = tslaX[-trainIndexHL_Goog,]

XgoogTrain_SR = tslaX[trainIndexSR_Goog,] 
XgoogTest_SR  = tslaX[-trainIndexSR_Goog,]
...
#Supervisor = diffHighLowTomorrow
(tslaXTransTrain_HL = caret::preProcess(as.data.frame(XtslaTrain_HL), method = c("BoxCox","center","scale","pca")))

#Apply the transformations("BoxCox","center","scale","pca")
pcaTslaXTrain_HL = stats::predict(tslaXTransTrain_HL,as.data.frame(XtslaTrain_HL)) #Outputs are PCA components
#head(pcaTslaX)

#Summary of PCA results
tslaXTransTrain_HL_       = caret::preProcess(as.data.frame(XtslaTrain_HL), method = c("BoxCox","center","scale"))
propVar_pcaTslaXTrain_HL_ = as.data.frame(XtslaTrain_HL) %>%
                 stats::predict(tslaXTransTrain_HL_,.) %>%
                 stats::prcomp() %>%
                 summary()
propVar_pcaTslaXTrain_HL_ #proportion of variance explained by each PCs

#Supervisor = stockReturnTomorrow
(tslaXTransTrain_HL = caret::preProcess(as.data.frame(XtslaTrain_HL), method = c("BoxCox","center","scale","pca")))
...
###TSLA
#Supervisor = diffHighLowTomorrow
tuneGridTslaKNN = expand.grid(k = c(1,2,3,4,5,10,20,30,40,50))
knnOutTsla = train(x = pcaTslaX[,1:5], y = tslaY$diffHighLowTomorrow,
                   method = "knn",
                   tuneGrid = tuneGridTslaKNN,
                   trControl = trainControl(method = 'cv', number = 10))
k_TslaKNN = knnOutTsla$finalModel$k

knnOutTsla$results$Rsquared
knnOutTsla$results$RMSE

tslaKNNTest

#Supervisor = stockReturnTomorrow
#tuneGridTslaKNN = expand.grid(k = c(1,2,10,50,100,180))
knnOutTsla = train(x = pcaTslaX[,1:5], y = tslaY$stockReturnTomorrow,
                   method = "knn",
                   tuneGrid = tuneGridTslaKNN,
                   trControl = trainControl(method = 'cv', number = 10))

diffHighLowTomorrowPred = predict(knnOutTsla, pcaTslaX[,1:5])
residTslaKNN_HL = tslaY$diffHighLowTomorrow - diffHighLowTomorrowPred
plot(tslaY$diffHighLowTomorrow, residTslaKNN_HL)
