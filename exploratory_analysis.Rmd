...
```{r assignYandX}
tslaAddCCIRemoveNA = tslaAddCCI %>%
  na.omit()

tslaY_    = dplyr::as_tibble(tslaAddCCIRemoveNA$stockReturn[2:nrow(tslaAddCCIRemoveNA)]) 
tslaY   = dplyr::as_tibble(tslaAddCCIRemoveNA$diffHighLow[2:nrow(tslaAddCCIRemoveNA)]) 


tslaX_    = tslaAddCCIRemoveNA %>% 
  #dplyr::select(-stockReturn) %>% Don't delete it. Use as a AutoRegression feature.
  .[1:(nrow(tslaAddCCIRemoveNA)-1),] %>%
  dplyr::as_tibble(.) 

tslaX   = tslaAddCCIRemoveNA %>% 
  #dplyr::select(-diffHighLow) %>% Don't delete it. Use as a AutoRegression feature.
  .[1:(nrow(tslaAddCCIRemoveNA)-1),] %>%
  dplyr::as_tibble(.) 

tslaFull_ = as_tibble(cbind(tslaY_,tslaX_))
tslaFull = as_tibble(cbind(tslaY,tslaX))

names(tslaFull_)[1] = "stockReturnTomorrow"
names(tslaFull)[1] = "diffHighLowTomorrow"


hclustPlotF(tslaFull_)
hclustPlotF(tslaFull)

names(tslaX_)[(cor(tslaY_,tslaX_)>=0.5)]
names(tslaX_)[which.max(cor(tslaY_,tslaX_))]

names(tslaX)[(cor(tslaY,tslaX)>=0.5)]
...
ggplot(data = pcaTslaX)+
  geom_point(aes(x = PC2, y=tslaY$value))

ggplot(data = pcaTslaX)+
  geom_point(aes(x = PC3, y=tslaY$value))
...
library(MASS)
tslaOutLogistic = glm(YtslaDummy ~ a+b+c, family = binomial, data = xx)

library(bestglm)
glmTslaBest = bestglm::bestglm(dataframeWithYatLast, family = binomial, IC="BIC")
