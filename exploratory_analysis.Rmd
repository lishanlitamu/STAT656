...
tslaAddVolYesterday = addLaggedVarF(tslaAddCCI$PX_VOLUME,tslaAddCCI, "diffVolume", 1)

tslaAddVolTemp = tslaAddVolYesterday %>%
  mutate(diffVolume = PX_VOLUME - diffVolume1) %>%
  dplyr::slice(-1) %>%
  mutate(increasedVolume = rep(0.01, nrow(.)))

for (i in 1:nrow(tslaAddVolTemp)) {
  if (tslaAddVolTemp$diffVolume[i] <=0 ) {
    tslaAddVolTemp$increasedVolume[i] = 0
} else {
  tslaAddVolTemp$increasedVolume[i] = tslaAddVolTemp$diffVolume[i] 
}
}
...
varPastVolumesF = function(featureVolume,lagsize) {
  ...
  for (d in 1:(length(featureVolume) - lagsize + 1)) {
    ...
    pastVol_upper[d+lagsize] = mean(pastVol) + 1.96*var(pastVol)**.5
  }
  return(pastVol_upper)
}

pastVol_upper = varPastVolumesF(tslaAddVolTemp$PX_VOLUME,5) #2373

pastVol_upper[1:20]

tslaAddVolTemp_ = tslaAddVolTemp %>%
  dplyr::slice(-1) %>%
  mutate(pastVol_upper) 
...
#Standardize two of three supervisors.We don't need to standardize a dummy variable.
YtslaNNScaledFit = caret::preProcess(YtslaPenalty,method = c("center","scale")) 
YtslaNNScaled    = predict(YtslaNNScaledFit,YtslaPenalty) 

YtslaNN = YtslaNNScaled %>% mutate(YtslaDummy)
...
#Standardize all 41 quantitative features.
XtslaNNScaledFit = caret::preProcess(XtslaNN,method = c("center","scale")) 
XtslaNNScaled    = predict(XtslaNNScaledFit,XtslaNN) 

#Add dummy variables back to the feature matrix.
XtslaNNFull      = XtslaNNScaled %>% mutate(tslaLogReg %>% select(c(aboveR1.Yes,aboveR2.Yes,aboveR3.Yes,belowS1.Yes,belowS2.Yes,belowS3.Yes))) 

#XtslaTrain = XtslaNNScaled[trainIndex,] #After standardization, selected as training data
#XtslaTest  = XtslaNNScaled[-trainIndex,]#After standardization, selected as test data

XtslaTrain = XtslaNNFull[trainIndex,] #After standardization, selected as training data
XtslaTest  = XtslaNNFull[-trainIndex,]#After standardization, selected as test data
```

```{r createDataForNN_Python}
...
marsVipTslaSR = vip::vip(marsTuneTslaSR, num_features = 41,  bar = NULL,geom = "point", value = "gcv")+ ggplot2::ggtitle("GCV - TSLA (47 Features)")
plot(marsVipTslaSR)
##Supervisor = diffHighLoW
marsVipTslaHL = vip::vip(marsTuneTslaHL, num_features = 41,  bar = NULL,geom = "point", value = "gcv")+ ggplot2::ggtitle("GCV - TSLA (47 Features)")
plot(marsVipTslaHL)
```

```{r removeCorrMARS}
### Compare results after removing highly correlated features. 

#Remove highly correlated features
highCorrTsla = highCorrF(XtslaNNFull,0.8)
highCorrTsla

XtslaNNFullRemoveCorr = XtslaNNFull %>% select(-highCorrTsla)
XtslaTrainRemoveCorr = XtslaNNFullRemoveCorr[trainIndex,]
XtslaTestRemoveCorr  = XtslaNNFullRemoveCorr[-trainIndex,]
...
#-1-# Supervisor = StockReturn
#MARS#
marsTuneTslaRemoveCorrSR = caret::train(
  x = XtslaTrainRemoveCorr,
  y = YtslaTrain_StockReturn,
  method = "earth",
  metric = "RMSE",
  trControl = trainControl(method = "cv", number = number),
...
marsTuneTslaRemoveCorrSR$bestTune
#library(digest) #Fixed error: Error: package ‘digest’ does not have a namespace
ggplot2::ggplot(marsTuneTslaRemoveCorrSR)

min(marsTuneTslaRemoveCorrSR$results$RMSE)
max(marsTuneTslaRemoveCorrSR$results$Rsquared)

#Multiple Linear Regression#
mlrCVTslaRemoveCorrSR = train(
  x = XtslaTrainRemoveCorr,
  y = YtslaTrain_StockReturn,
  method = "lm",
  metric = "RMSE",
...
mlrCVTslaRemoveCorrSR$results$RMSE
mlrCVTslaRemoveCorrSR$results$Rsquared

#Elastic Net#
glmnetCVTslaRemoveCorrSR = train(
  x = XtslaTrainRemoveCorr,
  y = YtslaTrain_StockReturn,
  method = "glmnet",
  trControl = trainControl(method = "cv", number = number),
...
min(glmnetCVTslaRemoveCorrSR$results$RMSE)
max(glmnetCVTslaRemoveCorrSR$results$Rsquared, na.rm = TRUE)




#-2-#Supervisor = diffHighLow
marsTuneTslaRemoveCorrHL = caret::train(
  x = XtslaTrainRemoveCorr,
  y = YtslaTrain_DiffHighLow,
  method = "earth",
  metric = "RMSE",
...
marsTuneTslaRemoveCorrHL$bestTune
ggplot2::ggplot(marsTuneTslaRemoveCorrHL)

min(marsTuneTslaRemoveCorrHL$results$RMSE)
max(marsTuneTslaRemoveCorrHL$results$Rsquared, na.rm = TRUE)



mlrCVTslaRemoveCorrHL = train(
  x = XtslaTrainRemoveCorr,
  y = YtslaTrain_DiffHighLow,
  method = "lm",
  metric = "RMSE",
...
mlrCVTslaRemoveCorrHL$results$RMSE
mlrCVTslaRemoveCorrHL$results$Rsquared

glmnetCVTslaRemoveCorrHL = train(
  x = XtslaTrainRemoveCorr,
  y = YtslaTrain_DiffHighLow,
  method = "glmnet",
  trControl = trainControl(method = "cv", number = number),
...
min(glmnetCVTslaRemoveCorrHL$results$RMSE)
max(glmnetCVTslaRemoveCorrHL$results$Rsquared, na.rm = TRUE)


#Variable Importance
##Supervisor = StockReturn
marsVipTslaRemoveCorrSR = vip::vip(marsTuneTslaRemoveCorrSR, num_features = 24,  bar = NULL,geom = "point", value = "gcv")+ ggplot2::ggtitle("GCV - TSLA (24 Features)")
plot(marsVipTslaRemoveCorrSR)
##Supervisor = diffHighLoW
marsVipTslaRemoveCorrHL = vip::vip(marsTuneTslaRemoveCorrHL, num_features = 24,  bar = NULL,geom = "point", value = "gcv")+ ggplot2::ggtitle("GCV - TSLA (24 Features)")
plot(marsVipTslaRemoveCorrHL)
```

``` {r summary(MARS vs MLR vs ElasticNet}
#Overall Comparisons of results from previous section

namesRMSE = c("RMSE_MARS_tslaStockReturn",
              "RMSE_MLR_tslaStockReturn",
              "RMSE_GLMNET_tslaStockReturn",
              "RMSE_MARS_tslaStockReturn_RemoveCorr",
              "RMSE_MLR_tslaStockReturn_RemoveCorr",
              "RMSE_GLMNET_tslaStockReturn_RemoveCorr",
              "RMSE_MARS_tslaDiffHighLow",
              "RMSE_MLR_tslaDiffHighLow",
              "RMSE_GLMNET_tslaDiffHighLow",
              "RMSE_MARS_tslaDiffHighLow_RemoveCorr",
              "RMSE_MLR_tslaDiffHighLow_RemoveCorr",
              "RMSE_GLMNET_tslaDiffHighLow_RemoveCorr")

summaryRMSE = c(min(marsTuneTslaSR$results$RMSE),
                mlrCVTslaSR$results$RMSE,
                min(glmnetCVTslaSR$results$RMSE),
                min(marsTuneTslaRemoveCorrSR$results$RMSE), 
                mlrCVTslaRemoveCorrSR$results$RMSE,
...
sumRMSE = cbind(namesRMSE,round(summaryRMSE,digits = 5))
...
namesRsquared = c("Rsquared_MARS_tslaStockReturn",
              "Rsquared_MLR_tslaStockReturn",
              "Rsquared_GLMNET_tslaStockReturn",
              "Rsquared_MARS_tslaStockReturn_RemoveCorr",
              "Rsquared_MLR_tslaStockReturn_RemoveCorr",
              "Rsquared_GLMNET_tslaStockReturn_RemoveCorr",
              "Rsquared_MARS_tslaDiffHighLow",
              "Rsquared_MLR_tslaDiffHighLow",
              "Rsquared_GLMNET_tslaDiffHighLow",
              "Rsquared_MARS_tslaDiffHighLow_RemoveCorr",
              "Rsquared_MLR_tslaDiffHighLow_RemoveCorr",
              "Rsquared_GLMNET_tslaDiffHighLow_RemoveCorr")
summaryRsquared = c(max(marsTuneTslaSR$results$Rsquared),
                mlrCVTslaSR$results$Rsquared,
                max(glmnetCVTslaSR$results$Rsquared, na.rm = TRUE),
                max(marsTuneTslaRemoveCorrSR$results$Rsquared), 
                mlrCVTslaRemoveCorrSR$results$Rsquared,
                ...
                mlrCVTslaHL$results$Rsquared,
                max(glmnetCVTslaHL$results$Rsquared, na.rm = TRUE),
                max(marsTuneTslaRemoveCorrHL$results$Rsquared),
                mlrCVTslaRemoveCorrHL$results$Rsquared,
                max(glmnetCVTslaRemoveCorrHL$results$Rsquared, na.rm = TRUE))


sumRsquared = cbind(namesRsquared, round(summaryRsquared,digits = 5))
