...
As we have both daily and quarterly updated features aligned in the original data set, quarterly updated features contains mainly missing values. Instead of those features, missing values are interpolated between quarters.   
...
XtslaLogRegRemoveCorr = XtslaPenalty %>% select(-highCorrTsla)

#ridge regression
ridgeOutTsla = train(x = XtslaLogRegRemoveCorr, y = YtslaPenalty$diffHighLowTomorrow, method = "ridge", tuneLength = 10,trControl = trControl)

ridgeOutTsla

#lasso regression
lassoOutTsla = train(x = XtslaLogRegRemoveCorr, y = YtslaPenalty$diffHighLowTomorrow, method = "lasso", tuneLength = 10,trControl = trControl)
