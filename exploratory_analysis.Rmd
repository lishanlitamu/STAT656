...
#####Dummy Variables
###TSLA
XtslaAddLevels = tslaFullStandardized %>%
...
XtslaWithSentiments = merge(XtslaWithoutDummy,tslaSentiments)
YtslaWithSentiments = merge(YtslaStandardizedWithDummy, tslaSentiments)[2:235,]
YtslaAligned        = YtslaWithSentiments %>%
                      select("diffHighLowTomorrow","stockReturnTomorrow",
                             "stockReturnTomorrowDummy")

colnames(YtslaWithSentiments)
tslaYnumeric = tslaY %>% select(diffHighLowTomorrow, stockReturnTomorrow)
YtslaStandardizedWithDummy = tslaYnumeric %>%
  caret::preProcess(method = c("center","scale")) %>%
  predict(newdata = tslaYnumeric) %>%
