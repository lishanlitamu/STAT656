...
#### 3.3 Yahoo Finance News

```{r yahoo_finance_news}
doc <- xml2::read_html("http://finance.yahoo.com/q/h?s=TSLA+Headlines")
scope <- doc %>% rvest::html_nodes("#latestQuoteNewsStream-0-Stream")
dates <- scope %>% rvest::html_nodes("cite span") %>% rvest::html_text()
headlines <- scope %>% rvest::html_nodes("h3 + ul") %>% 
  base::lapply(. %>% rvest::html_nodes("li a") %>% rvest::html_text)

# combine both
do.call(rbind,Map(cbind, dates, headlines))

//*[@id="latestQuoteNewsStream-0-Stream"]/ul/li[15]/div/div/div[2]/h3/a/text()
//*[@id="latestQuoteNewsStream-0-Stream"]/ul/li[11]/div/div/div[2]/h3/a/text()
```
...
Up till now, we added lagged features, OBV, CMF,EMA and RSI to our data. Need to clean it up before calculate the correlation of updated data by deleting rows with NAs. (Missing values are simultaneously created as we added past values of features to the data. Thus, we use listwise deletion instead of imputation.)

```{r cleanUp}
tslaReg = tslaLagged %>% na.omit()
```

```{r dummyVarForLogisticRegression}
tslaRegAddLevels = tslaReg %>%
  mutate(stockReturnDummy = factor(ifelse(stockReturn > 0, "Increased", "Decreased"))) %>%
  mutate(aboveR1PreviousDay = factor(ifelse(lag(PX_HIGH > R1), "Yes", "No"))) %>%
  mutate(aboveR2PreviousDay = factor(ifelse(lag(PX_HIGH > R2), "Yes", "No"))) %>%
  mutate(aboveR3PreviousDay = factor(ifelse(lag(PX_HIGH > R3), "Yes", "No"))) %>%
  mutate(belowS1PreviousDay = factor(ifelse(lag(PX_LOW  < S1), "Yes", "No"))) %>%
  mutate(belowS2PreviousDay = factor(ifelse(lag(PX_LOW  < S2), "Yes", "No"))) %>%
  mutate(belowS3PreviousDay = factor(ifelse(lag(PX_LOW  < S3), "Yes", "No"))) %>%
  select(stockReturnDummy,aboveR1PreviousDay, aboveR2PreviousDay, aboveR3PreviousDay, belowS1PreviousDay, belowS2PreviousDay, belowS3PreviousDay)

tslaDummyModel = caret::dummyVars(~ ., data = tslaRegAddLevels, fullRank = TRUE)

tslaRegDummyVar = as_tibble(predict(tslaDummyModel, tslaRegAddLevels))
YtslaDummy = tslaRegDummyVar$stockReturnDummy.Increased
XtslaDummy = select(tslaRegDummyVar, - stockReturnDummy.Increased)

tslaRegFull = tslaReg %>%
  mutate(YtslaDummy) %>%
  mutate(XtslaDummy) %>% 
  dplyr::select(-(136:205)) %>% #Drop PP, R1, R2, R3, S1, S2, S3 and their lagged values
  dplyr::select(-stockReturn) %>%  #replaced by stockReturnDummy
  na.omit()
#names(tslaRegFull) #Find positions of PP, R1, R2, R3, S1, S2, S3 and their lagged values

```

```{r removeHighCorr}
highCorrTsla = highCorrF(tslaRegFull,0.8)

tslaRegFullRemoveCorr = select(tslaRegFull, -highCorrTsla)

hclustPlotF(tslaRegFullRemoveCorr)
```
...
```{r logisticRegressionAllData}
YtslaFull = tslaRegFullRemoveCorr$YtslaDummy
XtslaFull = tslaRegFullRemoveCorr %>% select(-YtslaDummy, -diffHighLow,-diffLastOpen, -PX_VOLUME) # To predict day_0 stock trend, use stock price/volume data that happened before day_0.

names(tslaRegFullRemoveCorr)

trControl = trainControl(method = 'none')
tslaOutLogistic = train(x = XtslaFull, y = YtslaFull, method = 'glm', trControl = trControl)

summary(tslaOutLogistic)

#Forward Selection
tslaOutForward = leaps::regsubsets(x = XtslaFull, y = YtslaFull, nvmax=ncol(XtslaFull), method='forward')
...
XtslaForward = as_tibble(select(XtslaFull, rsi, cmf, cmf10))
...
##### 5.x Potential Problems
###### 5.x.1 correlation of error term(residuals)
###### 5.x.2 outliers
