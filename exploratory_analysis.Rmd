...
#Stock Return vs PCs
ggplot(data = pcaTslaX)+
  geom_point(aes(x = PC1, y=tslaY$stockReturn),color = ifelse(tslaY$stockReturn < 0  ,'red','green'))

ggplot(data = pcaTslaX)+
  geom_point(aes(x = PC2, y=tslaY$stockReturn),color = ifelse(tslaY$stockReturn < 0  ,'red','green'))

#There's no need to fit a PCA regression model, because the correlations between daily stock return and 9 PCs are almost zero. 
