...
As we have both daily and quarterly updated features aligned in the original data set, quarterly updated features contains mainly missing values. Instead of deleting quarterly updated features, missing values are interpolated between quarters.   
...
plot3D::scatter3D(tslaRegPCA$PC1, tslaRegPCA$PC2, tslaRegPCA$PC3, pch = 16,  
                  theta = 20, phi = 20, main = "PCA - Tesla", xlab = "PC1", 
...
```{r cleanUpForLogisticRegression}
tslaLogReg = tslaLaggedDiff %>%
  na.omit()
```
...
tslaLogRegAddLevels = tslaLogReg %>%
  mutate(stockReturnDummy = factor(ifelse(stockReturn > 0, "Increased", "Decreased"))) %>%
...
tslaDummyModel = caret::dummyVars(~ ., data = tslaLogRegAddLevels, fullRank = TRUE)

tslaLogRegDummyVar = as_tibble(predict(tslaDummyModel, tslaLogRegAddLevels))
YtslaDummy = tslaLogRegDummyVar$stockReturnDummy.Increased
XtslaDummy = select(tslaLogRegDummyVar, - stockReturnDummy.Increased)

tslaLogRegFull = tslaLogReg %>%
  mutate(YtslaDummy) %>%
  mutate(XtslaDummy) %>% 
  dplyr::select(-(136:205)) %>% #Drop PP, R1, R2, R3, S1, S2, S3 and their lagged values
  #names(tslaLogRegFull) #Find positions of PP, R1, R2, R3, S1, S2, S3 and their lagged values
  dplyr::select(-stockReturn) %>%  #replaced by stockReturnDummy
  select(-names(tslaLogReg)[1:20]) %>% #replaced by past values
...
```{r removeHighCorr}
highCorrTsla = highCorrF(tslaLogRegFull,0.8)

tslaLogRegFullRemoveCorr = select(tslaLogRegFull, -highCorrTsla)

hclustPlotF(tslaLogRegFullRemoveCorr)
```
...
YtslaFull = tslaLogRegFullRemoveCorr$YtslaDummy
XtslaFull = tslaLogRegFullRemoveCorr %>% select(-YtslaDummy) # To predict day_0 stock trend, use stock price/volume data that happened before day_0.
