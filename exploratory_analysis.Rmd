...
trControl = caret::trainControl(method = "repeatedcv", repeats = 2, 
                                number = 10, classProbs = TRUE)
#Y1
tuneGridTree_TslaHL = expand.grid(cp = c(0.001,0.01,0.1))
...
#Y2
tuneGridTree_TslaSR = expand.grid(cp = c(0.001,0.01,0.1))
rpartOut_TslaSR     = train(x = Xtrain_tslaSR, y = Ytrain_tslaSR,
                            method = "rpart",
...
#Y3
tuneGridRanger_TslaSRdummy = data.frame(splitrule = "variance", min.node.size = 5,
                                   mtry = round(sqrt(ncol(Xtrain_tslaSRdummy))))
rfOut_tslaSRdummy  = train(x = XtrainMat_tslaSRdummy, y = Ytrain_tslaSRdummy, 
                      method = "ranger",
                      num.trees = 500,
...
```{r reviewFeatureImportance}

rfImportance_TslaHL = rfOut_tslaHL$finalModel$variable.importance/sum(rfOut_tslaHL$finalModel$variable.importance)

glmnetImportance_TslaHL = abs(betaHatGlmnetTslaHL[-1])/sum(abs(betaHatGlmnetTslaHL[-1]))

colVec = rainbow(3)

plot(1:10, rfImportance_TslaHL[1:10], xlab = " ",
     xaxt = 'n', col = colVec[1], pch = 16, ylim = c(0, 0.5))
points(1:10,glmnetImportance_TslaHL[1:10], col = colVec[2], pch = 17)
points(1:10,boostImportance_TslaHL[1:10], col = colVec[3], pch = 18)
axis(1, labels = names(Xtrain_tslaHL)[1:10], at = 1:ncol(Xtrain_tslaHL[,1:10]), las = 3)
legend('topright', col = colVec, pch = 16:18, legend = c('rf','glmnet','boost'))

boostImportance_TslaHL[1:10]
names(Xtrain_tslaHL)[1:10]
